            ...
            try:
                if hasattr(sec(0.5), name):
                    if name[-4:] != '_ion':
                        ...
                        ionname = name[:-4]
                        result += [_neuron_var(name) for name in ['i' + ionname, ionname + 'i', ionname + 'o', 'e' + ionname]]
                    break
            except NameError:
                pass
    result += get_rxd_vars(secs)
    return [{'type': None, 'name': 'No coloration'}] + sorted([_neuron_var('cm'), _neuron_var('diam'), _neuron_var('i_cap'), _neuron_var('v')] + result, key=lambda n: n['name'].lower()) # _section_var('Ra')
